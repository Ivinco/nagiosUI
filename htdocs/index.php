...
function getNotesUrls() {
	...
	if (file_exists($getNotesUrls_cacheFile) and (time() - filemtime($getNotesUrls_cacheFile)) < 3600) return unserialize(file_get_contents($getNotesUrls_cacheFile));
...
function getDepends() {
        ...
        if (file_exists($getDepends_cacheFile) and (time() - filemtime($getDepends_cacheFile)) < 3600) return unserialize(file_get_contents($getDepends_cacheFile));
...
echo '	<alert state="'. $state .'" origState="'. $origState .'">
		<host>'.               $host .'</host>
		...
		<sched>'.              (int)$attrs['scheduled'] .'</sched>
		<downtime_id>'.        ((isset($ackAndSchedMatches[$host][$service]) && $state == 'OK') ? $ackAndSchedMatches[$host][$service]['downtime_id'] : '') .'</downtime_id>
		<sched_author>'.       (isset($ackAndSchedMatches[$host][$service]) ?                     $ackAndSchedMatches[$host][$service]['schedAuthor'] : '') .'</sched_author>
		...
		<attempt>'.            $attrs['attempts']/$attrs['max_attempts'] .'</attempt>
